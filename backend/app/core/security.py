...
def create_access_token(user_id: UUID, expires_delta: timedelta | None = None) -> str:
    ...
    encoded_jwt = jwt.encode(
        to_encode, settings.secret_key.get_secret_value(), algorithm=settings.jwt_algorithm
    )
    return encoded_jwt
...
def create_jwt_refresh_token(
    ...
    return RefreshTokenData(user_id=user_id, token=encoded_jwt, jti=jti, expires_at=expire)
...
def decode_access_token(token: str) -> dict[str, Any]:
    data = jwt.decode(
        ...
        options=jwt.types.Options(require=["sub", "jti", "type", "exp", "iat"]),
    )
    if data["type"] != "access":
